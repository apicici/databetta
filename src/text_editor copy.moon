    ...
    switch key
        ...
        when "backspace"
            if @end_cursor
                ...
                @delete(@pos, @end_pos)
            elseif @pos > 1
                if @modified ~= "delete" and not @history.current
                    @save_history()
                @modified = "delete"
                p = utf8.offset(@text, @pos)
        ...
        when "end"
            ...
            @cursor.cursor = line_len
            @pos = @cursor_to_pos(@cursor)
        when "lctrl", "rctrl", "lgui", "rgui", "c", "v", "z", "y"
            if love.keyboard.isDown("lctrl", "rctrl", "lgui", "rgui")
                ...
                elseif love.keyboard.isDown("z") 
                    if #@history > 0 and @history.current ~= #@history
                        if not @history.current or @modified
                            @modified = nil
                            @save_history()
                        @history.current = math.min((@history.current or 1) + 1, #@history)
                        @load_history()

                elseif love.keyboard.isDown("y")
                    if i = @history.current
                        if i > 1
                            @history.current = math.max(i - 1, 1)
                            @load_history()
...
M.textinput = (t) =>
    if @modified ~= "insert" and not @history.current
        @save_history()
    @modified = "insert"
    if @end_cursor
...
M.save_history = (action_type) =>
    t = {text:@text, pos:math.max(@pos, @end_pos or -1)}
    if i = @history.current
        @history = [h for h in *@history[i, MAX_HISTORY - 1]]
    table.insert(@history, 1, t)

M.load_history = () =>
    if t = @history[@history.current]
        @text = t.text
        @wrap_text()
        @end_cursor, @end_pos = nil
        @pos = t.pos
...
action_delete_backspace_S = Class {
    __init: (@pos, char) =>
        @chars = char
        @len = 1
